...
Recently I faced the task of renaming a set of files {`foo 02.jpg`, ..., `foo 74.jpg`} to {`foo 06.jpg`, ..., `foo 76.jpg`}. My approach contained nothing extraordinary:
```bash
#!/bin/bash
...
Yet there were some educational points in it:
 ...
 - One can use `$(  )` for executing a command and having bash treat the output as the source code. (It's the same as using backticks, as around `seq 74 2`, but allows nesting and is clearer. Kinda like `eval` in other languages, like JavaScript.) Not shown here, but it even works in quotation marks, e.g. `"$(echo hey yo)"` will be `"hey yo"`. Note that the trailing newline is deleted.
